"""HBI Control Center v2.1 - Fixed f-string syntax"""
...
def cmd_map_availability_price():
    ...
    if old not in txt:
        log("[FAIL] Pattern not found in facades.py")
        return
    ...
    if import_end == -1:
        log("[FAIL] Cannot find where to insert helpers")
        return
    backup(facades)
    ...
    if e2e.exists():
        rc, out, _ = run_cmd([sys.executable, str(e2e)])
        if "availability:   None" in out:
            log("[WARN] availability still None")
        else:
            log("[OK] availability and price mapped")
...
def cmd_save_status():
    ...
    lines.append("**By:** HBI Control Center v2.1")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("---")
    lines.append("")
    lines.append("*This file is auto-generated by HBI Control Center v2.1*")
    content = "\n".join(lines)
...
def cmd_cleanup():
    ...
    for f in ["inspect_dto.py","check_scoring.py","fix_test.py","fix_fixtures.py",
              ...
              "upgrade_dto.py","upgrade_facade.py","diagnostic.txt"]:
        p = P / f
        if p.exists():
            p.unlink()
            log("[DEL] " + f)
...
def cmd_setup_small_cmd():
    ...
    log("Double-click hbi_small.bat to open small CMD")
    log("Note: Windows does not allow programmatic window positioning.")
    log("To position: drag window to top-right corner.")
    log("To resize: right-click title bar -> Properties -> Layout")

MENU = """
+=============================================================+
|       HBI CONTROL CENTER v2.1                               |
+=============================================================+
...
def main():
    clear_screen()
    log("=== HBI Control Center v2.1 started ===")
    while True:
        print(MENU)
