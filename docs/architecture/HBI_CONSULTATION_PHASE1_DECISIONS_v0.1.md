...
| Field | Value |
|-------|--------|
| Status | **DRAFT — AWAITING INDEPENDENT REVIEW / PO MERGE** |
...
| **G2** | decision_state initializes `evidence_gaps` to `[]` at generate; no Case write timing exists. | **No V1 lifecycle timing for durable gap writes.** Computed unknowns/unmapped needs during generate remain in-memory signals only. | **DEFERRED** | `_build_decision_state` | — |
...
| **G4** | Durable gap persistence not used in consultation flow. | **Durable Evidence Gap persistence is NOT required for V1.** Concept remains documentary; runtime may expose insufficient/unmapped via existing decision_status / recommendation warnings without Case column writes. | **RESOLVED-CONTRACT** | Contract §2A + code search | Unblocks VS without fake persistence claims |
...
| OPEN requiring PO product preference (not blocked by missing code evidence) | **None material for starting VS** | O1–O5 and P1 closed from evidence; G1–G3 deferred with G4 resolved |

If PO later wants ProfileFact in Rec or durable gaps on Case, that is a **new mission**, not a silent expansion of V1.
...
| Layer | Content |
|-------|---------|
| **Current runtime capability** | Customer legacy profile + consultation merge → Case-owned generate → factors from concerns → needs → eligibility → Recommendation; optional Feedback; ProfileFact storage exists but unused by Rec; Case need/gap columns unused |
| **V1 contract (this addendum)** | Use the runtime path above; do not require ProfileFact in Rec; do not require Case need/gap persistence; do not require Problem/Assessment entities; min follow-up = Feedback row; no Home redesign |
| **Future architecture** | Optional ProfileFact adapter, durable decision snapshots, richer outcome domain, Case/Need-first Home — only with separate authorization |
...
Output:
  Zero or more Recommendation rows with eligibility/reasons/warnings
...
Runtime Evidence:
  API responses + DB rows for Case, Recommendation, optional Feedback
```

**Out of this unit:** Home redesign, ProfileFact wiring, gap writers, new entities, scoring changes.
...
4. Docs-only PR; merge unauthorized until independent review + PO.  
5. Exactly one smallest VS specified without implementing it.  

---

*End of Phase 1 Decision Addendum v0.1*
