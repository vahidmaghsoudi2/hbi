...
class RecommendationService(BaseService[Recommendation, RecommendationRepository]):
    ...
    def _build_decision_state(self, case_id: str, customer_profile: Dict) -> Dict[str, Any]:
        ...
        for c in raw_concerns:
            factors.append({"name": "concern", "value": c, "source": "customer_input", "validity": "DECLARED"})
    ...
    def _map_eligibility(self, engine_result, decision_state, need_match, product_unknowns=None) -> str:
    ...
    def _upsert_current_recommendation(
        self, *, case_id: str, product_id: str, need_match: float, evidence_score: float,
        eligibility: str, ranking_score: float, ranking_reasons: str, exclusion_reasons: str = "",
    ) -> Recommendation:
        ...
        for product in products:
            ...
            for ev in evidences:
                evidence_list.append({
                    "evidence_id": ev.evidence_id, "claim_id": ev.claim_id, "field": ev.field,
                    "claim_type": ev.claim_type, "source_reference": ev.source_reference,
                    "source_type": ev.source_type, "evidence_strength": ev.evidence_strength,
                    "qa_status": ev.qa_status, "claim": ev.claim,
                })
            pk_snapshot = {}
            if pk:
                pk_snapshot = {
                    "known_use_cases": pk.known_use_cases, "claimed_benefits": pk.claimed_benefits,
                    "contraindications": pk.contraindications, "ingredients": pk.ingredients,
                }
            engine_result = self.reasoning_engine.run(
                product_id=product.product_id, product_knowledge_snapshot=pk_snapshot,
                evidence_list=evidence_list, need_match=need_match,
                evidence_score=evidence_score, inventory_score=inventory_score,
            )
            product_unknowns: List[Dict[str, Any]] = []
            for u in engine_result.get("unknowns", []):
                sev = u.get("severity", "LOW")
                product_unknowns.append({
                    "field": u.get("field"), "unknown_priority": self._map_unknown_priority(sev),
                    "action": u.get("action"), "notes": u.get("notes"), "product_id": product.product_id,
                })
