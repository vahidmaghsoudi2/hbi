...
async def generate_recommendations(request: RecommendationRequest, db: Session = Depends(get_db)):
    ...
    dtos = rec_facade.generate(request.case_id, request.customer_profile)
    responses = []
    for dto in dtos:
        item = _to_dict(dto)
        if not isinstance(item, dict):
            item = dict(item) if item else {}
        inventory = inv_facade.get_by_product(item.get("product_id", ""))
        if inventory:
            item["availability"] = getattr(inventory, "availability", None)
            item["price"] = getattr(inventory, "sale_price_toman", None)
        else:
...
async def get_recommendations_by_case(case_id: str, db: Session = Depends(get_db)):
